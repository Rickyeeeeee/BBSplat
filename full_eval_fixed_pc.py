...
parser.add_argument("--pc", type=int, default=1000000)
...
def get_pc_name(point_count: int) -> str:
    if point_count >= 1000000:
        return f'{point_count//1000000}M'
    elif point_count >= 1000:
        return f'{point_count//1000}k'
    else:
        return f'{point_count}'

pc_name = get_pc_name(args.pc)
mipnerf360_output_path = f"/workspace/work/A2TG_Baselines/FixedPC_{pc_name}/MipNerf360/bbsplat"
tnt_output_path = f"/workspace/work/A2TG_Baselines/FixedPC_{pc_name}/tandt/bbsplat"
db_output_path = f"/workspace/work/A2TG_Baselines/FixedPC_{pc_name}/db/bbsplat"
mipnerf360_outdoor_scenes = ["bicycle", "garden", "stump"]
mipnerf360_outdoor_points = [args.pc for i in range(len(mipnerf360_outdoor_scenes))]
mipnerf360_indoor_scenes = ["room", "counter", "kitchen", "bonsai"]
mipnerf360_indoor_points = [args.pc for i in range(len(mipnerf360_indoor_scenes))]
tanks_and_temples_scenes = ["truck", "train"]
tanks_and_temples_points = [args.pc for i in range(2)]
deep_blending_scenes = ["drjohnson", "playroom"] 
deep_blending_points = [args.pc for i in range(2)] 
